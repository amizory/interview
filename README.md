...
->  top, htop, lsmem, lscpu, free
    CPU - LoadAverage - очередь к процессор (процессорное время) число/количество ядер, среднее
    значение количества процессов, ожидающих выполнения в очереди процессора, за определенный 
    период времени. Этот показатель рассчитывается на основе информации о процессах, которые
    находятся в состоянии ожидания (waiting) или готовы к выполнению (runnable). 
    1, 5 и 15 минут
...
->  В Linux процессы могут находиться в нескольких состояниях, которые определяют
    ...
    операции ввода/вывода с диска, "беспробудном сне" - ожидает дискового ввода/вывода
    ...
    I (Idle): Процесс находится в состоянии ожидания, обычно в результате отсутствия работы для
    выполнения
...
->  stateless - scale 
    ...
    Haproxy - tcp/udp/balancer - трафик
    nginx - http (healthcheck?)
...
->  Транк - это физический или логический канал связи, который позволяет передавать несколько
    VLAN через одну физическую связь. VLAN - это логическая сеть, которая позволяет разделить
    физическую сеть на несколько независимых сетей. Каждая VLAN представляет собой отдельную
    сеть с собственным адресным пространством, маршрутизацией и политикой безопасности. ->
    фактически метка у пакета

    - Когда мы настраиваем транк на коммутаторе, мы указываем, какие VLAN должны быть переданы
    через этот транк. Коммутатор использует MAC-адреса устройств для определения, какой VLAN
    они принадлежат.
    - Когда устройство отправляет данные через транк, коммутатор добавляет тег VLAN (VLAN tag) к
    кадру данных. Этот тег содержит номер VLAN, к которой принадлежит устройство. Коммутатор на
    другом конце транка читает этот тег и направляет данные в соответствующую VLAN.
    - Коммутаторы используют MAC-адресное обучение (MAC address learning) для определения,
    какие устройства находятся в каждой VLAN. Когда устройство присоединяется к сети,
    коммутатор записывает его MAC-адрес в таблицу MAC-адресов. Эта таблица содержит информацию
...
->  Федерация на уровне keycloak -> способ синхронизации "модульный" -> внутреннее хранилище 
    пользователей -> объединение записей
```
...
```txt
5 - отсутствие подключения к сети у pod

->  В поде ничего нет необходимого, доступ нужен изнутри
    - apk, apt, dnf, pacman 
    - mnt бинарь -> curl
    - nsenter -> soft с хоста внутри тех же namespace
    - busybox (сетевой ns) -> scratch
```

```txt
6 - 2 ingress

->  Должно быть разные трафики, port-mapping
    ingress - L7 - для трафика - HTTP-URL-proxy - заголовки
...
->  minimal = пустой = 1 слой - FROM scratch
    ++ CMD/ENTRYPOINT
```
